...
public class generalManagerS : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (!isCreated)
        {
            ...
            saver = transform.GetComponent<saverS>();
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Backspace) && SceneManager.GetActiveScene().buildIndex != 0) EndLevel();

        if (Input.GetKeyDown(KeyCode.Escape) && SceneManager.GetActiveScene().buildIndex == 0) Application.Quit();
            ...
            for (int i = 1; i < 3; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    switch (i)
                    {
                        ...
                        case 2: GUILayout.Label(((CautiousnessSubTask)j).ToString(), GUILayout.Width(110)); break;
    ...
    public static void StartLevel()                                         // 3- When level started
    {
        if (SceneManager.GetActiveScene().buildIndex == 0)
        {
            ...
            SceneManager.LoadScene((int)ActiveSkill);
        }
    }
